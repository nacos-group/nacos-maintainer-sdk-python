...
一个示例项目，演示如何在 FastAPI 服务中接入 [Nacos](https://nacos.io/) 配置中心与服务发现，同时挂载 MCP（Model Context Protocol）工具链。项目使用 [PDM](https://pdm.fming.dev/) 管理依赖，目标 Python 3.13。
...
## 准备 Nacos 配置
1. 将仓库根目录的 `nacos-fastapi-mcp-example.yaml` 上传到 Nacos 配置中心：
...
## 额外提示
- `.env.example` 默认开启 Nacos 注册；若本地调试只想跑 FastAPI，可关闭 `MCP_REGISTER_ENABLED` 或调低 `APP_WORKERS`。
