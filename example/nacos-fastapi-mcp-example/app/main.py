...
from app.nacos_registry import BasicNacosRegistrar, mount_mcp
...
def _get_app_version() -> str:
    ...
    try:
        ...
        return pkg_version("nacos-fastapi-mcp-example")
    except PackageNotFoundError:
        return "0.1.0"

@asynccontextmanager
async def lifespan(app: FastAPI):
    # 1) MCP 装配（拿到 mcp 实例和 tools）
    mcp = mount_mcp(app)
    tools = getattr(mcp, "tools", None)

    # 2) Nacos 配置监听 & 服务注册（同步）
    ...
    # 3) MCP 注册到 Nacos（异步可 await）
    enabled = settings.MCP_REGISTER_ENABLED
    app_version = _get_app_version()
    try:
        await BasicNacosRegistrar.register_mcp(
        ...
        logger.error(f"register_mcp failed: {e}")
